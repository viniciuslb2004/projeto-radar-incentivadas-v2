"""Gera o .xlsx formatado exportado pela area interna (/interno-artica -- Busca e
Operacoes salvas, ver webapp/main.py::busca_exportar/salvos_exportar). Reativado a
partir do codigo arquivado em docs/archive/removed-features.md (secao 2.2, mesmo
conteudo, so o gate de quem pode chamar mudou -- ver webapp/admin/auth.py::
exigir_staff nas rotas que usam este modulo).

As linhas exportadas pela Busca vem PRONTAS do front-end (ultimosResultados, ja
renderizado na tela) -- o backend nunca re-roda a busca aqui, so monta a planilha
em cima do que foi passado. Isso garante que o arquivo bate exatamente com o que a
pessoa viu, mesmo que o banco mude entre o clique em "Buscar" e o clique em
"Exportar". As Operacoes salvas vem direto de `webapp/salvos.py::
listar_operacoes_salvas`.
"""
import io
...
def gerar_xlsx_operacoes_salvas(linhas: list) -> bytes:
    return _gerar_xlsx("Operações salvas", COLUNAS_SALVAS, linhas)
